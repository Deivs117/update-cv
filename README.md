...
- **`db:generate` + `db:migrate` es el flujo real para cualquier cambio de schema desde #91** (decisión explícita, ver el issue): entre #8 y #15 todos los cambios se habían aplicado con `db:push`, que no deja ningún registro (la tabla `drizzle.__drizzle_migrations` seguía vacía pese a que el schema ya tenía las 5 migraciones aplicadas). Se reconcilió una sola vez con `npm run baseline-migrations` (issue #91, no un comando de uso repetido — bautiza el historial ya aplicado sin re-ejecutar SQL, mecanismo exacto documentado en el propio script). `db:push` sigue disponible para iterar rápido en un schema todavía sin datos reales (ej. prototipando un cambio grande antes de generar la migración final), pero cualquier cambio que se aplique a dev/preview/producción debe pasar por `db:generate` (crea el archivo `.sql`) seguido de `db:migrate` (lo aplica Y deja el registro en `__drizzle_migrations`) — nunca `db:push` directo contra un entorno con datos reales de ahora en más.
